...
Notes:
- On Windows, Microsoft Word is used when available. On Render, Word documents are processed with Aspose.Words. The Render configuration pins Python 3.12 and installs Aspose.Words as a required dependency; PDF files are processed with PyMuPDF.
- The included `Dockerfile` is an optional deployment alternative that adds Calibri-compatible and Arial-compatible fonts. Use it when document layout depends on those fonts; exact parity with a desktop still requires the original document fonts to be installed or embedded.
